(function () {
  ...
  function renderFinance(){const host=$('financeContent');if(!finance){host.innerHTML='<div class="suite-empty"><h2>预算待接入</h2><p>接入核对后的资金计划后，可在这里查看月度预算与还款安排。</p></div>';return;}
    ...
    const openingView='<strong>'+money(d.opening.availableCents)+'</strong>'+(d.opening.accounts?.length?'<p class="finance-accounts" aria-label="可用余额合计的组成">'+d.opening.accounts.map(a=>'<span>'+escape(a.name)+' '+money(a.amountCents)+'</span>').join('<i aria-hidden="true">·</i>')+'</p>':'');
    const pinPrefix='置顶提示：',rules=Array.isArray(d.rules)?d.rules:[],pinned=rules.filter(r=>typeof r==='string'&&r.startsWith(pinPrefix)&&r.slice(pinPrefix.length).trim());
    const notice=pinned.length?'<section class="suite-card finance-notice" aria-labelledby="financeNoticeTitle"><h2 id="financeNoticeTitle">特别提示 · 给我自己看</h2>'+pinned.map(r=>'<p>'+escape(r.slice(pinPrefix.length))+'</p>').join('')+'</section>':'';
    host.innerHTML=notice+'<div class="finance-kpis"><article class="finance-baseline"><span>已核对可用余额</span>'+openingView+'<small>截至 '+escape(d.opening.asOf)+' · 非实时余额</small></article><article><span>基准日锁定资金</span><strong>'+money(d.opening.lockedCents)+'</strong><small>单列，不计入上面的可用余额</small></article><article class="'+(gaps.length?'finance-shortfall':'')+'"><span>'+(gaps.length?'最大月末缺口':'预测最低月末结余')+'</span><strong>'+money(lowest?Math.abs(lowest.balanceCents):undefined)+'</strong><small>'+escape(lowest?.month||'暂无月份')+(gaps.length?' · '+gaps.length+' 个月预计有缺口':' · 按现有预算')+'</small></article></div>'+
      ...
      '<details class="suite-card finance-rules"><summary>账本来源与计算说明</summary><p>来源：'+escape(d.source.version)+' · '+escape(d.source.file)+'</p><p>Excel 为主账本，网页显示最近一次同步的预算。</p><ul>'+rules.filter(r=>!pinned.includes(r)).map(r=>'<li>'+escape(r)+'</li>').join('')+'</ul></details>';
